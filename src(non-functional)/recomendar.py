...
def carregar_dados_produtos():
    conn = conectar_bd()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, price, genre_id FROM products")
...
def preprocessar_dados(produtos):
    df = pd.DataFrame(produtos, columns=["id", "name", "price", "genre_id"])
    df['products'] = pd.Categorical(df['genre_id']).codes
